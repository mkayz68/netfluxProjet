<?php
    ...
    if(isset($_POST['creerSerie']))
    {
        ...
        {      
            ...
            if(!empty($_POST['imageSynop']))
            {
                ...
                elseif($_FILES['imageSynop']['error'] == 0)
                {
                    ...
                    if($extImage !== FALSE)
                    {
                        ...
                        $imageSource = $fonction->moveImage('Synopsis', $extensionUpload, $_FILES['imageSynop']['tmp_name']);
            ...
            {
                $tabMessage[] = 'Vous devez ajouter une image pour le synopsis de la série';
            }
        }
    ...
    if(isset($_POST['creerArticle']))
    {
        ...
        if ($envoiBdd)
        {
            ...
            if ($donnees == 1 AND !empty($_POST['imageArticle']))
            { 
                ...
                if($extImage !== FALSE)
                {
                    ...
                    $imageSource = $fonction->moveImage('Article', $extensionUpload, $_FILES['imageArticle']['tmp_name']);
            ...
            elseif ($donnees == 2 AND !empty($_POST['imageArticle']))
            {
    ...
    if(isset($_POST['modifierSerie']))
    {
        ...
        $idSerie = $tabSerie[count($tabSerie)-1];
        $nomSeries = str_replace($tabSerie[count($tabSerie)-1], "", $tabSerie);
        $nomSerieString = implode(" ", $nomSeries);
        $nomS = htmlspecialchars(trim(str_replace(" ","", $nomSerieString)));
        ...
        else
        {
            ...
            elseif (!empty($nomSerie))
            {
                $serieManager->updateByChamps($idSerie, array('nomSerie'), array($nomSerie));

                $imageModifSynop = $link->deleteLinkRedim('Synopsis', 200, $nomS);
                $imageModifArticle = $link->deleteLinkRedim('Article', 200, $nomS);
                $nomSerie = htmlspecialchars(trim(str_replace(" ","", $nomSerie)));
                $imageNewNom = $link->deleteLinkRedim('Synopsis', 200, $nomSerie);
                rename($imageModifSynop, $imageNewNom);
                rename($imageModifArticle, $imageNewNom);
            }
            ...
            if (!empty($_POST['imageSynop']))
            {
                ...
                elseif($_FILES['imageSynop']['error'] == 0)
                {
                    if(empty($nomSerie))
                    { 
                        $image = $imageManager->getListImageSynopsis($idSerie);
                        $imageEffacer = $link->deleteLinkImage($image['image'], 'Synopsis');
                        unlink($imageEffacer);
                        $imageEffacer = $link->deleteLinkRedim('Synopsis', 200, $nomS);
                        unlink($imageEffacer);
                    }
                    elseif(!empty($nomSerie))
                    {
                    ...
                    if($extImage !== FALSE)
                    {
                        ...
                        $fonction->resizeImage($imageSource[0], 200, $nomSerie, 'Synopsis');

                        $image = $imageSource[1];
                        $imageManager->updateByChamps1($idSerie, array('image'), array($image));
    ...
    if(isset($_POST['modifierArticle']))
    {
        ...
        $auteurArticle = htmlspecialchars(ucfirst(trim($_POST['auteurArticle'])));
        $articleSerie = htmlspecialchars($_POST['articleSerie']);
        $tabArticle = explode(" ", $_POST['article']);
        $idSerie = $tabArticle[count($tabArticle)-1];
        $idArticle = $tabArticle[0];
        $nomSeries = explode("/", $_POST['article']);
        $nomSeries = str_replace($nomSeries[0], "", $nomSeries);
        $nomSeries = explode(" ", $nomSeries[1]);
        $nomSeries = str_replace($nomSeries[count($nomSeries)-1], "", $nomSeries);
        $nomSerieString = implode(" ", $nomSeries);
        $nomS = htmlspecialchars(trim(str_replace(" ","", $nomSerieString)));
        ...
        //Conditions pour l'image
        if (!empty($_POST['imageArticle']))
        {
            if($_FILES['imageArticle']['error'] > 0)
            {
                $tabMessage[] = 'Erreur lors du transfert';
            }    
            elseif ($_FILES['imageArticle']['error'] == 0)
            {
                $image = $imageManager->getListImageArticle($idSerie);
                $imageEffacer = $link->deleteLinkImage($image['image'], 'Article');
                unlink($imageEffacer);
                $imageEffacer = $link->deleteLinkRedim('Article', 500, $nomS);
                unlink($imageEffacer);
                ...
                if($extImage !== FALSE)
                {
                    ...
                    $image = $imageSource[1];
                    $imageManager->updateByChamps2($idSerie, array('image'), array($image));
    ...
    if(isset($_POST['supprSerie']))
    {
        ...
        $tabMessage = [];
            ...
            if(!empty($idSerie))
            {    
                //Effacer l'image synopsis du dossier images
                $image = $imageManager->getListImageSynopsis($idSerie);
                $imageEffacer = $link->deleteLinkImage($image['image'], 'Synopsis');
                unlink($imageEffacer);
                $imageEffacer = $link->deleteLinkRedim('Synopsis', 200, $nomS);
                ...
                //Effacer l'image article du dossier article
                $image = $imageManager->getListImageArticle($idSerie);
                $imageEffacer = $link->deleteLinkImage($image['image'], 'Article');
                unlink($imageEffacer);
                $imageEffacer = $link->deleteLinkRedim('Article', 500, $nomS);
                ...
                //Effacer les traces de la série dans toute la BDD
